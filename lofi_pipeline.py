    ...
    if sub_page == "Queue":
        ...
        for row in visible:
            ...
            with col_main:
                ...
                st.markdown(
                    f"**{row['name']}** &nbsp; {_rec_score_badge(score)}"
                    f"{network_str}"
                )
        ...
        for row in rows:
            ...
            with c_info:
                badge = "pending scrape" if row.get("needs_scraping") else "scraped"
                st.write(
                    f"**{row['name']}** — {badge} &nbsp; {_rec_score_badge(score)}"
                )
                parts = []
                if v := (cm.get("sp_monthly_listeners") or cm.get("spotify_listeners")):
                    parts.append(f"SP {_fmt(v)}")
                if v := (cm.get("ig_followers") or cm.get("instagram_followers")):
                ...
                if genres_raw:
                    parts.append("  ·  ".join(str(g) for g in genres_raw[:3]))
                if parts:
                    st.caption("  ·  ".join(parts))
            st.divider()
